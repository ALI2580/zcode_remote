...
void main() {
  ...
  testWidgets(
      'review: MCP scope picker initially represents the supplied current monitor',
      (tester) async {
    ...
    try {
      ...
      expect(find.text('MCP for C'), findsOneWidget);
  ...
  testWidgets(
      'review: old MCP workspace opening cannot replace a later selection',
      (tester) async {
    tester.view.devicePixelRatio = 1;
    tester.view.physicalSize = const Size(1180, 900);
    addTearDown(tester.view.reset);
    ...
    try {
      ...
      expect(find.text('MCP for B'), findsOneWidget);
      fixture.gateA.complete();
      await tester.pumpAndSettle();
      expect(find.text('MCP for B'), findsOneWidget);
      expect(find.text('MCP for A'), findsNothing);
