...
void main() {
  testWidgets(
      'review: model settings refresh existing main and side composer catalogs',
      (tester) async {
                  ...
                  'options': [
                    {
                      ...
                      'name': (provider['models'] as List).first['name'],
    ...
    try {
      ...
      await tester.pumpAndSettle();
      // The current SettingsCenter opens the selected model's editor from
      // the row-level Edit action.  Keep this review on the model catalog
      // write path rather than the provider metadata dialog.
      final name = find.byWidgetPredicate((widget) =>
          widget is TextField && widget.decoration?.labelText == 'Model name');
      await tester.enterText(name, 'After change');
      await tester.tap(find.text('Save'));
      await tester.pumpAndSettle();
      expect((provider['models'] as List).first['name'], 'After change');
      for (final controller in controllers) {
        ...
        expect(controller.input.text, 'Preserve ${controller.sessionId} draft');
      }
